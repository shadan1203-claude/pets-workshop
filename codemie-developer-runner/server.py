...
def run_claude(command, prompt, cwd=None, timeout=3600):
    """
    Run CodeMie Claude and provide the task through stdin.

    The CodeMie AI agent recursion limit is explicitly configured
    through AI_AGENT_RECURSION_LIMIT.
    """
    ...
    try:
        ...
        # Windows support for .cmd/.bat executables.
        if os.name == "nt":
            resolved = shutil.which(executable)
        ...
        # --------------------------------------------------------
        # CodeMie AI Agent configuration
        # --------------------------------------------------------

        env = os.environ.copy()

        # Default to 500 if not explicitly configured.
        recursion_limit = os.getenv(
            "AI_AGENT_RECURSION_LIMIT",
            "500",
        )

        env["AI_AGENT_RECURSION_LIMIT"] = recursion_limit
...
"""
        ...
        #     --task task
        #
        # because your CodeMie/Claude CLI reported:
        #
